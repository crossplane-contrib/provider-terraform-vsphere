...
func (e *ctyDecoder) DecodeCty(mr resource.Managed, ctyValue cty.Value, schema *providers.Schema) (resource.Managed, error) {
	...
	return DecodeTag(r, ctyValue)
}

func DecodeTag(prev *Tag, ctyValue cty.Value) (resource.Managed, error) {
	valMap := ctyValue.AsValueMap()
	new := prev.DeepCopy()
	DecodeTag_CategoryId(&new.Spec.ForProvider, valMap)
	DecodeTag_Description(&new.Spec.ForProvider, valMap)
	DecodeTag_Name(&new.Spec.ForProvider, valMap)
...
func DecodeTag_CategoryId(p *TagParameters, vals map[string]cty.Value) {
	p.CategoryId = ctwhy.ValueAsString(vals["category_id"])
}
